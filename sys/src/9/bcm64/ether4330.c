...
//	{ 0x4345, 6, "cyfmac43455-sdio.bin", "brcmfmac43455-sdio.txt", "cyfmac43455-sdio.clm_blob" },
...
sdiorwext(int fn, int write, void *a, int len, int addr, int incr)
{
	...
	while(len > 0){
		...
		if(blk)
			sdwifilink->iosetup(write, a, bsize, bcount);
		else
			sdwifilink->iosetup(write, a, bcount, 1);
		sdiocmd_locked(IO_RW_EXTENDED,
			write<<31 | (fn&7)<<28 | blk<<27 | incr<<26 | (addr&0x1FFFF)<<9 | (bcount&0x1FF));
		sdwifilink->io(write, a, m);
		qunlock(&sdiolock);
		poperror();
		len -= m;
		a = (char*)a + m;
...
sdioinit(void)
{
	...
	int i;
	uchar alts[2][6] = {
		{ Alt4, Alt4, Alt4, Alt3, Alt4, Alt3 },		/* C0, C1 stepping */
		{ Alt1, Alt1, Alt1, Alt1, Alt1, Alt1 },		/* D0 stepping */
	};

	/* connect sdhci to wifi */
	for(i = 30; i <= 35; i++){
		gpiosel(i, alts[soc.dstepping][i-30]);
		if(i == 30)
			gpiopulloff(i);
		else
			gpiopullup(i);
...
fwload(Ctlr *ctl)
{
	...
	if(FWDEBUG) print("firmware load %s...", firmware[i].fwfile);
...
intwait(Ctlr *ctlr, int wait)
{
	...
	for(;;){
		...
		if(i == 0){
			if(0) print("INTS(%lux)\n", cfgreadl(Fn1, ctlr->sdregs + Intstatus));
...
wlreadpkt(Ctlr *ctl)
{
	...
	for(;;){
		...
		if(len > SdpcmSize)
			packetrw(0, b->wp + SdpcmSize, len - SdpcmSize);
		b->wp += len;
		break;
	}
	qunlock(&ctl->pktlock);
...
rproc(void *a)
{
	...
	for(;;){
		...
		case 0:
			...
			if(BLEN(b) < SdpcmSize + sizeof(Cmd))
				break;
			q = (Cmd*)(b->rp + SdpcmSize);
...
etherbcmifstat(Ether* edev, void* a, long n, ulong offset)
{
	...
	/* these strings are known by 9aux/wpa */
