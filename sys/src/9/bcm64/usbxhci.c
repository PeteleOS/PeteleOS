...
#ifndef USEPCI
	intrenable(hp->irq, interrupt, hp, UNKNOWN, "usbxhci");
#endif
}
...
waittd(Ctlr *ctlr, Wait *w, int tmout)
{
	...
	*r->doorbell = r->id;
	...
	poperror();
	{ char *err;
	  err = ccerrstr(w->er[2]>>24);
	  if(err != nil && strcmp(err, "???") == 0)
		iprint("waittd (from %#p) unknown error %d\n", getcallerpc(&ctlr), w->er[2]>>24);
	  return err;
	}
}
...
#else
static void
scanctlr(int i, uintptr io, int irq)
{
	Ctlr *ctlr;
	if(ctlrs[i])
		return;
...
reset(Hci *hp)
{
	...
	scanpci();
#else
	scanctlr(0, VIRTIO+0x80200000ull, IRQusbhxci1);
	scanctlr(1, VIRTIO+0x80300000ull, IRQusbxhci2);
#endif
