...
static void parse_inst(void)
{
    ...
    if (op == LMOVW || op == LMOVB)
    {
        ...
        op = yylval.lval;
        {
            ...
            if (yypeek(0) == LREG || yypeek(0) == LR)
            {
                ...
                {
                    yyget();
                    if (yypeek(0) == LCREG || yypeek(0) == LCR || yypeek(0) == LSPREG || yypeek(0) == LSPR || yypeek(0) == LMSR || yypeek(0) == LLR || yypeek(0) == LCTR || yypeek(0) == LFPSCR)
                    {
                        yyhave = sh;
                        for (i = 0; i < sh; i++)
                        {
    ...
    movw_reg_first:;
        ...
        t = yypeek(0);
        if (t == LREG || t == LR || t == LFREG || t == LF)
        {
            int sh2, i2;
            long st2[256];
            YYSTYPE sv2[256];
            sh2 = yyhave;
            for (i2 = 0; i2 < yyhave; i2++)
            {
                st2[i2] = yytok[i2];
                sv2[i2] = yyval[i2];
            }
            ...
            if (yypeek(0) == ',')
            {
                yyget();
                t = yypeek(0);
                if (t == LCREG || t == LCR || t == LSPREG || t == LSPR || t == LMSR || t == LLR || t == LCTR || t == LFPSCR || t == LFP || t == LF || t == '(' || t == LNAME || isconstart(t))
                {
                    yyhave = sh2;
                    for (i2 = 0; i2 < sh2; i2++)
                    {
    ...
    movw_general:;
        {
            Gen f1;
            int isf1, isf2;
            isf1 = (yypeek(0) == LFREG || yypeek(0) == LF);
            if (isf1)
                f1 = parse_freg();
            else if (yypeek(0) == LREG || yypeek(0) == LR)
                f1 = parse_rreg();
            else if (yypeek(0) == LFPSCR)
            {
                if (yypeek(1) == '(')
                    f1 = parse_fpscrf();
                else
            ...
            else if (yypeek(0) == LCREG || yypeek(0) == LCR)
                f1 = parse_creg();
            else if (yypeek(0) == LSPREG || yypeek(0) == LSPR || yypeek(0) == LMSR)
                f1 = parse_psr();
            else if (yypeek(0) == LLR)
                f1 = parse_lr();
            else if (yypeek(0) == LCTR)
                f1 = parse_ctr();
            else if (yypeek(0) == '$')
                f1 = parse_imm();
            else
                f1 = parse_addr();
            yyexpect(',');
            if (yypeek(0) == LFPSCR && yypeek(1) != '(')
            {
                b = parse_fpscr();
                outcode(op, &f1, NREG, &b);
                return;
            }
            if (yypeek(0) == LFPSCR)
            {
            ...
            if ((yypeek(0) == LCREG || yypeek(0) == LCR))
            {
                b = parse_creg();
                outcode(op, &f1, NREG, &b);
                return;
            }
            if (yypeek(0) == LSPREG || yypeek(0) == LSPR || yypeek(0) == LMSR || yypeek(0) == LLR || yypeek(0) == LCTR)
            {
                if (isf1 && (f1.type == D_FREG))
                {
                    b = parse_psr();
                    if (b.type == D_SPR || b.type == D_MSR)
                    {
                        outcode(op, &f1, NREG, &b);
                        return;
                    }
            ...
            if (yypeek(0) == '$' && op == LMOVW)
            {
                ...
                if (yypeek(0) == ',')
                {
                    yyget();
                    c = parse_fpscr();
                    outgcode(op, &f1, NREG, &b, &c);
                    return;
                }
                outcode(op, &f1, NREG, &b);
                return;
            }
            isf2 = (yypeek(0) == LFREG || yypeek(0) == LF);
            if (isf2)
            {
                b = parse_freg();
                outcode(op, &f1, NREG, &b);
    ...
    case LADDW:
    {
        ...
        if (yypeek(0) == '$')
        {
            a = parse_imm();
            yyexpect(',');
            if ((yypeek(0) == LREG || yypeek(0) == LR) && yypeek(1) == ',')
            {
                s = parse_sreg();
                yyexpect(',');
                b = parse_rreg();
    ...
    case LCROP:
        ...
        a = parse_cbit();
        yyexpect(',');
        if (yypeek(0) == LCONST || yypeek(0) == LVAR || yypeek(0) == '-' || yypeek(0) == '+' || yypeek(0) == '~' || yypeek(0) == '(')
        {
            int sh, i;
            long st[256];
            YYSTYPE sv[256];
    ...
    case LBRA:
    {
        ...
        if (yypeek(0) == ',')
        {
            ...
            if (isconstart(yypeek(0)) || yypeek(0) == LNAME || yypeek(0) == LLAB)
            {
                int sh, i;
                long st[256];
                YYSTYPE sv[256];
        ...
        if (yypeek(0) == ',')
        {
            ...
            if (op == LFMA)
            {
                ...
                yyexpect(',');
                {
                    Gen d;
                    d = parse_freg();
                    outgcode(op, &a, b.reg, &c, &d);
                    return;
                }
            }
            c = parse_freg();
            if (op == LFCMP && yypeek(0) != ',' && yypeek(0) != ';' && yypeek(0) != 0 && yypeek(0) != EOF)
            {
            }
            if (op == LFCMP && (c.type == D_CREG || yypeek(0) == ','))
            {
                if (c.type != D_CREG)
                {
                    Gen cr;
                    cr = parse_creg();
                    outcode(op, &a, cr.reg, &b);
                    return;
                }
            }
    ...
    case LRLWM:
        ...
        b = parse_rreg();
        yyexpect(',');
        if ((yypeek(0) == LCONST || yypeek(0) == LVAR || yypeek(0) == '-' || yypeek(0) == '+' || yypeek(0) == '~' || yypeek(0) == '(') && yypeek(1) == ',')
        {
            c = parse_imm();
            yyexpect(',');
            {
    ...
    case LXOP:
    {
        ...
        if (op == LXLD)
        {
            a = parse_regaddr();
            yyexpect(',');
            if (yypeek(0) == '$' || ((yypeek(0) == LCONST || yypeek(0) == LVAR) && 0))
            {
            }
            if (isconstart(yypeek(0)) || yypeek(0) == '$')
            {
                int sh, i;
                long st[256];
                YYSTYPE sv[256];
        ...
        if (op == LXST)
        {
            ...
            if (yypeek(0) == '$' || isconstart(yypeek(0)))
            {
                int sh, i;
                long st[256];
                YYSTYPE sv[256];
    ...
    case LWORD:
        yyget();
        op = yylval.lval;
        if (yypeek(0) == '$' && (yypeek(1) == LSCONST || 0))
        {
            b = parse_ximm();
        }
        else if (yypeek(0) == '$')
