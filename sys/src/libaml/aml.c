...
Vfmt(Fmt *f)
{
	...
	case 'L':
		...
		e = r->ref;
		if(c == 'A')
			return fmtprint(f, "Arg%lld=%V", r->ptr - e->arg, *r->ptr);
		if(c == 'L')
			return fmtprint(f, "Local%lld=%V", r->ptr - e->loc, *r->ptr);
	case 'n':
...
dumpregs(void)
{
	...
	e = nil;
	for(f = FP; f >= FB; f--){
		print("%.8p.%.2lux: %-8s %N\t", f->start, (ulong)(f-FB), f->phase, f->dot);
		if(f->op)
			print("%s", f->op->name);
		print("(");
		for(i=0; i<f->narg; i++){
...
xec(uchar *pc, uchar *end, Name *dot, Env *env, void **pret)
{
	...
	for(;;){
		if((++loop & 127) == 0)
			gc();
		if(amldebug)
			print("\n%.8p.%.2lux %-8s\t%N\t", PC, (ulong)(FP - FB), FP->phase, FP->dot);
		r = nil;
		c = *FP->phase++;
		switch(c){
		default:
